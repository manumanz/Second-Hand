...
(function () {
  ...
  const MOTIONS = [
    ...
    { id: 'heavy',   label: 'walks like an apology',        sway: 1.0,  jolt: 1.5,  speed: 0.85 },
    { id: 'quick',   label: 'always six minutes late',      sway: 1.1,  jolt: 1.1,  speed: 1.3 },
    { id: 'tired',   label: 'carries the week in the knees', sway: 0.9, jolt: 0.9,  speed: 0.8 },
  ];

  const OCCUPATIONS = [
    { id: 'office',    sched: 'office',    filler: ['receipt', 'pencap'],      line: 'the days are shaped like desks.' },
    { id: 'barista',   sched: 'barista',   filler: ['sachet', 'receipt'],      line: 'they smell of steamed milk by nine.' },
    { id: 'courier',   sched: 'courier',   filler: ['receipt', 'coin'],        line: 'the whole city, twice a day, at speed.' },
    { id: 'student',   sched: 'student',   filler: ['eraser', 'usb'],          line: 'everything is due thursday. everything.' },
    { id: 'bartender', sched: 'bartender', filler: ['bottlecap', 'napkin'],    line: 'they come home when the birds start.' },
    { id: 'gardener',  sched: 'gardener',  filler: ['seedpacket', 'twine'],    line: 'there is always soil in my seams. i have made peace.' },
    { id: 'nurse',     sched: 'nurse',     filler: ['pencap', 'gum'],          line: 'they sleep while the world is loud, work while it is not.' },
  ];

  const NEW_OWNERS = [
    'a night-shift cleaner who liked the colour',
    'a drummer who needed something warm for the walk home',
    'a retired postman who reads found receipts like novels',
    ...
    'an old man who buys a coat every decade, needed or not',
  ];

  const STREETS = ['maple', 'harrow', 'birch', 'foundry', 'chapel', 'penny lane', 'old mill'];

  /* ---- epilogue lines for ordinary survivors, found by the new owner ---- */
  const FOUND = {
    coin:       ['exact bus fare, on a morning it mattered.'],
    key:        ['a key to a door that may no longer exist. they keep it anyway. everyone does.'],
    receipt:    ['one coffee, ages ago. the postman was right: receipts are novels.'],
    ...
    pick:       ['a guitar pick worn to matte. they don’t play. they start.'],
    stamps:     ['stamps! actual stamps. someone gets a real letter this year.'],
    ...
    lighter:    ['a lighter, low on fuel. birthday candles, mostly, from now on.'],
    treat:      ['a cat treat. the new owner does not have a cat. a cat, sensing this, is already on its way.'],
    bottlecap:  ['a bottle cap from a good night. they can tell.'],
    seedpacket: ['half a packet of marigolds. the window box says thank you.'],
    sachet:     ['brown sugar, slightly fused. it stirs into a monday coffee.'],
    eraser:     ['an eraser worn round. mistakes were made, then unmade.'],
    ...
    cigpack:    ['an empty packet, crumpled with intent. a trophy, actually.'],
    ...
    twine:      ['a hand’s length of green twine. it will hold something up. it always does.'],
  ...
  /* =======================================================================
     ...
     hands: {f, seek, action:'peek'|'peeklong'|'take'|'give', flag, missFlag}
     a day may be a function(g) => dayObject so late days can react to flags.
     ======================================================================= */
  ...
  ARCS.push({
    ...
    days: [
      { lines: g => [
          'new coat smell is long gone. we are old friends, this coat and i.',
      ...
      { lines: () => [
          ...
          'a napkin came in, folded twice, with a phone number that is already saved.',
          'old love, i think. the kind you keep the paper of anyway.'],
        drops: [{ f: .4, type: 'napkin' }, { f: .7, type: 'coin' }] },
      { lines: () => [
          'the pacing started after lunch. four hundred steps, the same eight metres.',
          'and then — something small and heavy joined us. it has its own box.',
      ...
      { lines: () => [
          ...
          'they practised words under their breath. i felt them through the lining, all consonants and courage.'],
        drops: [{ f: .5, type: 'gum' }],
        hands: [{ f: .65, seek: 'ringbox', action: 'peek' }] },
      { lines: () => [
          'the box went away this morning and the small heavy came back alone.',
          'loose. rolling. i do not like how it rolls.',
          'if you have ever wanted to be useful, little pocket, be useful now.'],
        hands: [
      ...
      g => ({
        lines: (g.flags.ringLost)
          ? ['today was meant to be the day. they dressed carefully. i could tell by the ironing.',
             'the fingers came down for the ring and found lint and old fare.',
             'i have never heard a person go so quiet from the inside.']
          : ['today is the day. they dressed carefully — i could tell by the ironing.',
             'the fingers will come for the small heavy tonight. keep it where they can find it.',
             'or don’t. i’m a pocket, not a conscience.'],
      ...
      g => {
        if (g.flags.ringTaken) return {
          lines: ['they came home at 4am smelling of rain and someone else’s perfume.',
                  'the steps were dancing. even the lint is celebrating.',
                  'i am to be donated, i hear. new life, thinner coat. i understand completely.'],
          drops: [{ f: .3, type: 'coin' }, { f: .5, type: 'bottlecap' }],
          mods: { joy: 1 } };
        return {
          lines: ['they searched me four times today. i gave them everything i had, which was not the thing.',
                  'tonight they said the words anyway, with empty hands held open.',
    ...
    outcome: g => {
      ...
      if (f === 'lost')
        return ['somewhere on ' + g.street + ' street, a gold ring waits in a drain',
                'to be someone’s luckiest morning. the question got asked anyway, with open hands. it worked.'];
      if (f === 'in-pocket')
  ...
  ARCS.push({
    ...
    days: [
      { lines: g => [
          'a letter today. folded four times, soft at the creases.',
          'it has been carried before. letters that get carried are the unfinished kind.',
          g.occ.line],
        drops: [{ f: .3, type: 'letter' }, { f: .6, type: 'coin' }] },
      { lines: () => [
          'stamps. two of them, bought singly, the way you buy things you might not use.',
          'the letter got heavier overnight. that is not physics. i checked.'],
        drops: [{ f: .45, type: 'stamps' }],
        hands: [{ f: .7, seek: 'letter', action: 'peek' }] },
      { lines: () => [
          'we stood in front of the postbox for eleven minutes today.',
          'the fingers held the letter at the mouth of me for a long time. the wind came in.',
      ...
      { lines: g => [
          g.weatherWord === 'rain.' ? 'rain got into everything today. i kept the letter dry. it’s what i’m for.'
                                    : 'a long day of small errands. the letter rode along for all of them.',
          'i can feel one word through the paper where the pen pressed hardest. it says “sorry”.'],
        drops: [{ f: .5, type: 'gum' }] },
      { lines: () => [
          'they walked to the postbox again. faster this time, before the doubt could pack a bag.',
          'the fingers are coming for it. this is the moment, little pocket.',
          'let it go up, or keep it safe down here in the dark. both are a kind of love.'],
        hands: [{ f: .55, seek: 'letter', action: 'take', flag: 'letterMailed', missFlag: 'letterKept' }],
        mods: { pace: 1 } },
      g => ({
        lines: g.flags.letterMailed
          ? ['light today. lighter than the coat has been in months.',
             'they bought a pastry and ate it walking, which i understand is how joy travels.']
          : ['the letter is still with us. they think they lost it, or they let themselves think that.',
             'the steps were heavy but honest. some words need longer in the dark.'],
        drops: [{ f: .4, type: 'coin' }, { f: .65, type: 'receipt' }] }),
      g => ({
        lines: g.flags.letterMailed
          ? ['an old photograph came in today: two people on a wall, squinting.',
             'it has been in a drawer a long time. it wanted air. things are moving.',
             'i am to be donated. the coat of the before-times. fair enough.']
          : ['they wrote a second letter tonight — i heard the pen through the lining.',
             'maybe that one goes further. maybe it says the word out loud.',
             'i am to be donated tomorrow. whatever i still hold, i hold.'],
  ...
  ARCS.push({
    ...
    days: [
      { lines: g => [
          'a crumpled packet came in today, crushed on purpose. a declaration.',
          'and the lighter. still here. declarations are complicated.',
          g.occ.line],
        drops: [{ f: .25, type: 'cigpack' }, { f: .4, type: 'lighter' }] },
      { lines: () => [
          'gum. so much gum. the pocket economy has changed regimes overnight.',
          'twice today the fingers came down and touched the lighter and went away empty.',
          'i counted. i’m good at counting. it’s mostly what i do.'],
        drops: [{ f: .2, type: 'gum' }, { f: .6, type: 'gum' }],
        hands: [{ f: .45, seek: 'lighter', action: 'peek' }] },
      { lines: () => [
          'a bad day. i could tell by the jaw — clenching travels all the way down.',
          'the fingers will come hunting tonight. hide the fire or hand it over.',
          'i’m not here to judge. i’m here to hold things. but i notice you can hold things AWAY.'],
        drops: [{ f: .3, type: 'gum' }],
        hands: [{ f: .7, seek: 'lighter', action: 'take', flag: 'cave1', missFlag: 'resist1', returns: true }],
        mods: { pace: 1 } },
      g => ({
        lines: g.flags.cave1
          ? ['one, they said. just the one. the lighter came back down warm.',
             'nobody smokes one. but nobody quits in a straight line either.']
      ...
      { lines: () => [
        ...
        mods: { runjoy: 1 } },
      { lines: () => [
          'the last hard night, i think. old friends, a loud bar, everyone outside in the cold.',
          'the fingers will come once more, out of habit shaped like hunger.',
          'whatever you’ve been doing with that lighter — once more.'],
      ...
      g => ({
        lines: (g.flags.resist1 && g.flags.resist2)
          ? ['a whole week. they told the mirror this morning; i heard it through the wool.',
             ...
             'i am to be donated — a coat that doesn’t smell of smoke, for whoever comes next.']
          : ['not a straight line, this week. but the packet stayed crumpled, and that counts.',
             'they bought a small cake for no one’s birthday. progress tastes like that sometimes.',
             'i am to be donated. carry the news gently, whoever gets me.'],
    ...
    outcome: g => {
      ...
      if (f === 'lost')
        return ['the lighter fell out somewhere on ' + g.street + ' street.',
                'they took it as a sign. signs work, if you let them.'];
      if (resisted === 2)
        return ['day thirty-one, someone buys them a small cake.',
  ...
  ARCS.push({
    ...
    days: [
      { lines: () => [
          'they sang on the corner of foundry street today, and the sky mostly held.',
          'coins rained in like slow applause. and the picks — the lucky one is the worn one.',
          'you’ll know it. it’s the one shaped like all the songs it has already played.'],
        drops: [{ f: .15, type: 'pick', opts: { lucky: true } }, { f: .3, type: 'pick' }],
        mods: { perform: 1 } },
      { lines: () => [
          'a napkin came in with writing pressed hard enough to feel: “call about friday.”',
          'they read it six times. i felt every unfolding.',
          'friday, it seems, is an audition. fridays usually are.'],
        drops: [{ f: .4, type: 'napkin' }],
        mods: { perform: 1 } },
      { lines: () => [
          'practice, practice, a bus ticket to the rehearsal room, practice.',
      ...
      { lines: () => [
          ...
          'coins left. the picks stayed. priorities, correctly ordered.'],
      ...
      { lines: () => [
          ...
          'the way you carry a full cup across a room.'],
        drops: [{ f: .5, type: 'gum' }] },
      { lines: () => [
          'friday. the hands were cold all day — i could feel it every time they dove in.',
          'before the door, they’ll reach for the lucky pick. the worn one.',
          'keep it findable. or make them find their luck elsewhere — that works too, but it’s louder.'],
        hands: [{ f: .6, seek: 'pick', action: 'take', flag: 'pickTaken', missFlag: 'pickMissed', wantLucky: true }],
        mods: { pace: 1 } },
      g => ({
        lines: g.flags.pickTaken
          ? ['they played it clean. i could hear it in the walk home: 4/4, swinging.',
             'starting monday, someone gets paid to sing.',
             'a warmer coat is coming, apparently. i regret nothing.']
          : ['the pick wasn’t there at the door. a breath. a coin from the lining instead.',
             'they played with a coin like the old blues men, and it worked anyway.',
    ...
    outcome: g => {
      ...
      if (f === 'lost')
        return ['the lucky pick is in a drain on ' + g.street + ' street, still lucky.',
                'the audition went fine with a coin. luck is a rumour musicians spread about themselves.'];
      if (f === 'in-pocket')
        return ['the new owner finds a worn pick and, eventually, a guitar to match it.',
                'luck, it turns out, is transferable.'];
  ...
  ARCS.push({
    ...
    days: [
      { lines: g => [
          'something followed them home last night. something small, loud, and certain.',
          'today a cat treat arrived in me, which tells you how negotiations are going.',
          g.occ.line],
      ...
      { lines: () => [
          ...
          'ah. the small loud certain thing already belongs to somebody.'],
      ...
      { lines: () => [
          ...
          'give it up, and the cat goes home to whoever engraved that hope.',
          'hide it — or feed it to the hole — and the cat stays. i am just a pocket. you are the weather.'],
      ...
      g => ({
        lines: g.flags.tagCalled
          ? ['margot went home today. the flat is quieter than quiet.',
             'they walked the long way back, hands deep in me the whole time.',
             'grief for something you had a week. hearts are enormous. pockets know their sizes.']
          : ['a new collar arrived — bought, this time. no engraving yet.',
             'they are arguing with themselves about names. the cat has clearly already chosen its own.'],
      ...
      g => ({
        lines: g.flags.tagCalled
          ? ['this morning they went to the shelter and came home with the loudest kitten in the building.',
             'a kitten needs a warmer coat than me to ride in. i’m to be donated. good trade.']
          : ['the cat is called sunday now. today is its name-day, obviously.',
             'i am to be donated — apparently sunday sheds enough for two coats.',
             'whatever is left in me, i pass along with the fur. all of it non-negotiable.'],
        drops: [{ f: .4, type: 'treat' }] }),
    ],
    outcome: g => {
      ...
      if (f === 'lost')
        return ['the tag is under a hedge on ' + g.street + ' street.',
                'the cat stayed. some verdicts are delivered by the holes in pockets.'];
      if (f === 'in-pocket')
  ...
  ARCS.push({
    ...
    days: [
      { lines: g => [
          ...
          'also the door key, which suddenly feels like a guest instead of a resident.',
          g.occ.line],
      ...
      { lines: () => [
          'box day. eleven boxes. i felt every single stair, twice.',
          'a life fits in eleven boxes and one pocket. i’m carrying the important half, obviously.'],
        drops: [{ f: .5, type: 'twine' }],
        mods: { heavy: 1 } },
      { lines: () => [
          'today the door key goes back. the fingers will come for it.',
          'keys are the hardest goodbye that fits in a fist.'],
      ...
      { lines: () => [
          'last night in the old city. a long slow walk past all the usual ghosts.',
      ...
      g => ({
        lines: g.flags.ticketUsed
          ? [] // filled below, hands resolve first
    ...
    outcome: g => {
      if (g.flags.ticketUsed)
        return ['the platform was cold. the new city is warmer.',
                'the coat, they decided, belongs to the old one. somewhere north, box eight of eleven is being unpacked.'];
  ...
  SH.generateStranger = function (seed) {
    ...
    const g = {
      ...
      flags: {},
      items: [],      // filled by sim: {id,type,fate,...}
      lostYesterday: 0,
      weatherWord: '',
      fateOf(type) {
        let best = 'none';
  ...
  SH.resolveDay = function (g, n) {
    ...
    // everyday rummages: the fingers come counting fare most days
    ...
    if (lost.length)
      cards.push({ head: 'AND THE HOLE',
        lines: ['the hole kept ' + lost.length + ' small thing' + (lost.length > 1 ? 's' : '') + ' for itself.',
                'a drain on ' + g.street + ' street is quietly wealthy now.',
